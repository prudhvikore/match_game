...
class App extends Component {
  state = {score: 0, timerRunning: true}
  ...
  renderEndGame = () => {
    ...
    return (
      <div className="score-bg">
        ...
        <button className="play-again" type="button" onClick={this.resetGame}>
          <img
  ...
  render() {
    const {score, activeTabId} = this.state
    const {timerRunning} = this.state

    return (
      <div className="bg-container">
        <Navbar score={score} timerFinished={this.timerFinished} />
        {timerRunning ? this.renderGame(activeTabId) : this.renderEndGame()}
      </div>
    )
  }
}
